...
#define TOLERANCE    1.01  /* 1% */
  ...
  for (i = 0; i < numberof (t); i++)
      ...
      for (;;)
	{
	  ...
	  if (t[i] == 0.0)
	    {
	      ...
	      if (zeros > max_zeros)
		{
		  LOG_ERR ( "Fatal error: too many (%d) failed measurements (0.0)\n", zeros);
		  abort ();
		}
	     if (s->reps < 10000)
	       s->reps *= 2;
	  ...
	  if (reps_d > 2e9 || reps_d < 1.0)
	    {
	      LOG_ERR ( "Fatal error: new reps bad: %.2f\n", reps_d);
	      LOG_ERR ( "  (old reps %u, unittime %.4g, precision %d, t[i] %.4g)\n",
  ...
  LOG_ERR ( "speed_measure() could not get %d results within %.1f%%\n",
	   e, (TOLERANCE-1.0)*100.0);
  LOG_ERR ( "    unsorted         sorted\n");
  LOG_ERR ( "  %.12f    %.12f    is about 0.5%%\n",
...
speed_operand_src (struct speed_params *s, mp_ptr ptr, mp_size_t size)
{
  if (s->src_num >= numberof (s->src))
    {
      LOG_ERR ( "speed_operand_src: no room left in s->src[]\n");
      abort ();
    }
...
speed_operand_dst (struct speed_params *s, mp_ptr ptr, mp_size_t size)
{
  if (s->dst_num >= numberof (s->dst))
    {
      LOG_ERR ( "speed_operand_dst: no room left in s->dst[]\n");
      abort ();
    }
